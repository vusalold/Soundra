...
import com.vm.soundra.repository.DownloadHistoryRepository
...
class Mp3DownloadViewModel(context: Context) : ViewModel() {
    ...
    private fun setupPlayer() {
        ...
        val basePlayer = exoPlayer ?: return
        ...
        MediaSessionHolder.mediaSession = androidx.media3.session.MediaSession.Builder(appContext, forwardingPlayer)
            .build()
    }
    ...
    private suspend fun internalPlayTrack(
        ...
        withContext(Dispatchers.Main) {
            ...
            val currentSong = _currentQueue.value.find { it.id == trackId }
            val mediaMetadata = currentSong?.let {
                androidx.media3.common.MediaMetadata.Builder()
                    .setTitle(it.title)
                    .setArtist(it.uploaderName)
                    .setArtworkUri(android.net.Uri.parse(it.thumbnailUrl))
                    .build()
            ...
            if (mediaMetadata != androidx.media3.common.MediaMetadata.EMPTY) {
                mediaItemBuilder.setMediaMetadata(mediaMetadata)
            }
            val mediaItem = mediaItemBuilder.build()

            try {
                val serviceIntent = android.content.Intent(appContext, PlaybackService::class.java)
                ...
                Log.e("PLAYER", "Failed to start PlaybackService", e)
            }
    ...
    override fun onCleared() {
        MediaSessionHolder.mediaSession?.let { session ->
            session.release()
        }
        MediaSessionHolder.mediaSession = null
        exoPlayer?.release()
